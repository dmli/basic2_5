...
/**
 * Created by ldm on 10-6-11. 提供系统所需基本数据及功能
 */
public class SystemTool {
	...
	public static void killBackgroundProcesses(Context context) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (apInfo.pkgList != null && apInfo.pkgList.length > 0) {
				String[] pkgList = new String[apInfo.pkgList.length];
				System.arraycopy(apInfo.pkgList, 0, pkgList, 0, apInfo.pkgList.length);
				if (apInfo.importance > ActivityManager.RunningAppProcessInfo.IMPORTANCE_SERVICE) {
					for (String s : pkgList) {
						if ("com.android.providers".equals(s)) {
							continue;
						}
						if (SystemTool.DENSITY_DPI >= Build.VERSION_CODES.FROYO) {
	...
	/**
	 * 这个方法可能会更快，但建议使用dipToPx(Context, float)
	 * 
	 * @param value dip值
	 * @return px值
	 */
	public static float dipToPx(float value) {
		return value * DENSITY <= 0 ? Resources.getSystem().getDisplayMetrics().density : DENSITY;
	}
	...
	public static boolean hideSoftInput(final Context context, final IBinder binder) {
		...
		try {
			if (imm.isActive()) {
				...
				if (!bool && context != null && context instanceof Activity) {
	 ...
	 */
	public static void remoreActivity(Class<? extends Activity> cla) {
		final Map<String, WeakReference<Activity>> acs = activitySet;
		if (acs != null && acs.size() > 0) {
			String[] names = null;
			synchronized (acs) {
	 ...
	 */
	public static void remoreActivity(String activityKey) {
