...
public class MultiSecurityHandler<T extends Context> extends Handler{
	
	protected int index;
	protected WeakReference<Context> t = null;

	public MultiSecurityHandler(Context context, int index) {
		this.t = new WeakReference<Context>(context);
		this.index = index;
	}
	...
	private void handleMessage(Message msg, Context context, RequestSet requestSet, RequestRetCallBack callBack) {
		...
		} else if (msg.what == InternetEntity.RESULT_SUCCESS) {// 子线程“success”的回调
			callBack.success(callBack.data);
		} else if (msg.what == InternetEntity.RESULT_RET_NULL) {// 子线程“retNull”的回调
			callBack.retNull(context);
		} else if (msg.what == InternetEntity.RESULT_ERROR) {// 子线程“error”的回调
			callBack.error(context, callBack.code, String.valueOf(callBack.data));
		} else if (msg.what == InternetEntity.RESULT_IS_NETWORK_STATE) {// 子线程“isNetWorkState”的回调
			callBack.isNetWorkState(context);
		} else if (msg.what == InternetEntity.RESULT_IO_ERROR) {// 子线程“ioError”的回调
			callBack.ioError(context);
		} else if (msg.what == InternetEntity.RESULT_CHILD_ENTER) {// 子线程开始
			callBack.enter();
		} else if (msg.what == InternetEntity.RESULT_CHILD_EXIT) {// 多任务使用的子线程结束
			callBack.exit();
		}
	}
}
