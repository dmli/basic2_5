...
/**
 * ImageView Tonuch 引用网络作者Laladin.SYD源码 使用方法:
 *           MImageBrowser mib = new MImageBrowser(context, 540, 960);
 *           调用mib.init方法为其设置图片，将mib对象add到界面布局中
 */
public class LImageBrowser extends ImageView {
	...
	public LImageBrowser(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
	}

	public LImageBrowser(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public LImageBrowser(Context context) {
		super(context);
	}
	 ...
	 * @param node 父节点
	 */
	public void init(Bitmap bm, final View node) {
		setImageBitmap(bm);
		bmWidth = bm.getWidth();
		...
		new Handler().postDelayed(new Runnable() {
			@Override
			public void run() {
				screenW = node.getWidth();
				screenH = node.getHeight();
			}
		}, 500);
	}
	...
	public boolean ReScale() {
		...
		ScaleAnimation scaleanim = new ScaleAnimation(scaleX, 1f, scaleY, 1f, ScaleAnimation.RELATIVE_TO_SELF, piovtX, ScaleAnimation.RELATIVE_TO_SELF, piovtY);
	...
	private void rebound(int disX, int disY) {
		this.layout(getLeft() + disX, getTop() + disY, getLeft() + disX + getWidth(), getTop() + disY + getHeight());
	}
}