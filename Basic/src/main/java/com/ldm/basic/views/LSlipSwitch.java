...
public class LSlipSwitch extends RelativeLayout {
		...
		if (dragView.getLeft() > (this.getWidth() - dragView.getWidth()) / 2) {
			if (dragView.getLeft() != this.getWidth() - dragView.getWidth()) {
				...
				if (isTriggerCallback && onStateChangeListener != null) {
					onStateChangeListener.onStateChangeListener(LSlipSwitch.this.getId(), dragView, 1.0f);
			...
			if (dragView.getLeft() != 0) {
				...
				if (isTriggerCallback && onStateChangeListener != null) {
					onStateChangeListener.onStateChangeListener(LSlipSwitch.this.getId(), dragView, 0.0f);
					isTriggerCallback = false;
				}
				switchState(0);
			}
	...
	public interface OnStateChangeListener {
		 ...
		 */
        void onStateChangeListener(final int viewId, final View dragView, final float position);
	}
	...
	public interface OnPositionChangeListener {
		 ...
		 */
        void onPositionChangeListener(final View dragView, final float position);
	}
	...
	public interface OnViewClickListener {
		 ...
		 * @param position 位置 0.0左边 1.0右边
		 */
        void onViewClickListener(final float position);
	}
}
